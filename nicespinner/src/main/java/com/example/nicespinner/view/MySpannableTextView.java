 ...
 *
 * 注意：在使用时xml文件的    android:layout_width="match_parent"  要设置成“match_parent”
 */

public class MySpannableTextView extends AppCompatTextView {
    private int textCount = 80;//自定义字体个数，默认为80
    private int maxLine = 4;//自当以最大显示的行数，默认为4
    ...
    public MySpannableTextView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
       /* limitTextViewString(this.getText().toString(), textCount, this, new OnClickListener() {
            @Override
    ...
    private int getLastCharIndexForLimitTextView(TextView textView, String content, int width, int maxLine) {
        ...
        System.out.println("--------StaticLayout-----------"+staticLayout.getLineCount());
     ...
     */
    public void limitTextViewString(String textString, int maxFirstShowCharCount, final TextView textView, final View.OnClickListener clickListener) {
        //计算处理花费时间
        ...
        int lastCharIndex = getLastCharIndexForLimitTextView(textView, textString, width, maxLine);
//返回-1表示没有达到行数限制
